...
import { type MushafKey, type Theme, type FontSize, type DragHandlers } from "../types";
import quranData from "@/data/quran-data.json";

type SettingsSubView = "display" | "fontsize" | "mushaf" | "about" | "language" | "install" | null;
...
// The PANEL_INDEX values must match each panel's DOM order below.
const PANEL_COUNT = 7;
...
export function SettingsSheet({
  ...
  const panelCls = (active: boolean) =>
    `flex h-full w-[calc(100%/7)] flex-col transition-opacity duration-300 ${active ? "opacity-100" : "opacity-0 pointer-events-none"}`;
        ...
        >
          ...
          <div className={panelCls(settingsSubView === null)}>
            <div className="flex items-center justify-between border-b border-border px-5 pb-3 pt-2">
              <span className={`${scaleFont ? "text-[1.25rem]" : "text-[0.8125rem] tracking-[2px] uppercase"} font-semibold`}>{t(lang, "settings.title")}</span>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto">
              ...
              <button
                type="button"
                className="flex w-full items-center justify-between border-b border-border px-5 py-4 text-start active:bg-(--bg2)"
                onClick={() => setSettingsSubView("fontsize")}
              >
                <span className="text-base text-(--fg)">{t(lang, "settings.fontSize")}</span>
                <div className="flex items-center gap-1.5 text-(--fg2)">
                  <span className="text-sm">{t(lang, FONT_SIZE_LABEL_KEY[fontSize])}</span>
                  <ForwardChevron className="size-4" />
                </div>
              </button>
              <button
          ...
          <div className={panelCls(settingsSubView === "display")}>
            <div className="flex items-center justify-between border-b border-border px-5 pb-3 pt-2">
              <div className="flex items-center gap-2">
                ...
                <span className={`${scaleFont ? "text-[1.25rem]" : "text-[0.8125rem] tracking-[2px] uppercase"} font-semibold`}>{t(lang, "settings.theme")}</span>
              </div>
              <Button size="icon-sm" variant="ghost" className="rounded-full bg-(--bg2)" onClick={onClose}>
                <X className="size-4" />
              </Button>
          ...
          {/* Panel: font size picker */}
          <div className={panelCls(settingsSubView === "fontsize")}>
            <div className="flex items-center justify-between border-b border-border px-5 pb-3 pt-2">
              <div className="flex items-center gap-2">
                <Button size="icon-sm" variant="ghost" className="rounded-full bg-(--bg2)" onClick={() => setSettingsSubView(null)}>
                  <BackChevron className="size-4" />
                </Button>
                <span className={`${scaleFont ? "text-[1.25rem]" : "text-[0.8125rem] tracking-[2px] uppercase"} font-semibold`}>{t(lang, "settings.fontSize")}</span>
              </div>
              <Button size="icon-sm" variant="ghost" className="rounded-full bg-(--bg2)" onClick={onClose}>
                <X className="size-4" />
              </Button>
          ...
          <div className={panelCls(settingsSubView === "mushaf")}>
            <div className="flex items-center justify-between border-b border-border px-5 pb-3 pt-2">
              <div className="flex items-center gap-2">
                ...
                <span className={`${scaleFont ? "text-[1.25rem]" : "text-[0.8125rem] tracking-[2px] uppercase"} font-semibold`}>{t(lang, "settings.mushafStyle")}</span>
              </div>
              <Button size="icon-sm" variant="ghost" className="rounded-full bg-(--bg2)" onClick={onClose}>
                <X className="size-4" />
              </Button>
          ...
          <div className={panelCls(settingsSubView === "about")}>
            <div className="flex items-center justify-between border-b border-border px-5 pb-3 pt-2">
              <div className="flex items-center gap-2">
                ...
                <span className={`${scaleFont ? "text-[1.25rem]" : "text-[0.8125rem] tracking-[2px] uppercase"} font-semibold`}>{t(lang, "settings.about")}</span>
              </div>
              <Button size="icon-sm" variant="ghost" className="rounded-full bg-(--bg2)" onClick={onClose}>
                <X className="size-4" />
              </Button>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
              <p className="mb-6 text-[0.9375rem] text-(--fg2)">{t(lang, "about.versionLine", { version: appVersion })}</p>

              <div className="mb-6">
                <p className="mb-3 text-[0.75rem] font-semibold uppercase tracking-widest text-(--fg3)">{t(lang, "about.readingSection")}</p>
                <div className="flex flex-col gap-3">
                  <div>
                    <p className="text-[1.0625rem] font-medium text-(--fg)">{t(lang, "about.navigatePages")}</p>
                    <p className="mt-0.5 text-[0.9375rem] text-(--fg2)">{t(lang, "about.navigatePagesDesc")}</p>
                  </div>
                  <div>
                    <p className="text-[1.0625rem] font-medium text-(--fg)">{t(lang, "about.landscapeMode")}</p>
                    <p className="mt-0.5 text-[0.9375rem] text-(--fg2)">{t(lang, "about.landscapeModeDesc")}</p>
                  </div>
                  <div>
                    <p className="text-[1.0625rem] font-medium text-(--fg)">{t(lang, "about.toggleMenu")}</p>
                    <p className="mt-0.5 text-[0.9375rem] text-(--fg2)">{t(lang, "about.toggleMenuDesc")}</p>
                  </div>
                </div>
              </div>

              <div className="mb-6">
                <p className="mb-3 text-[0.75rem] font-semibold uppercase tracking-widest text-(--fg3)">{t(lang, "about.aiSection")}</p>
                <div className="flex flex-col gap-3">
                  <div>
                    <p className="text-[1.0625rem] font-medium text-(--fg)">{t(lang, "about.askFeature")}</p>
              ...
              <div className="mb-6">
                <p className="mb-3 text-[0.75rem] font-semibold uppercase tracking-widest text-(--fg3)">{t(lang, "about.highlightSection")}</p>
                <div className="flex flex-col gap-3">
                  <div>
                    <p className="text-[1.0625rem] font-medium text-(--fg)">{t(lang, "about.highlightLine")}</p>
                    <p className="mt-0.5 text-[0.9375rem] text-(--fg2)">{t(lang, "about.highlightLineDesc")}</p>
                  </div>
                  <div>
                    <p className="text-[1.0625rem] font-medium text-(--fg)">{t(lang, "about.addAnnotation")}</p>
                    <p className="mt-0.5 mb-2 text-[0.9375rem] text-(--fg2)">{t(lang, "about.addAnnotationDesc")}</p>
                    <p className="mt-0.5 text-[0.9375rem] text-(--fg2)">{t(lang, "about.addAnnotationTip")}</p>
                  </div>
                </div>
              </div>

              <div className="mb-6">
                <p className="mb-3 text-[0.75rem] font-semibold uppercase tracking-widest text-(--fg3)">{t(lang, "about.bookmarksSection")}</p>
                <div className="flex flex-col gap-3">
                  <div>
                    <p className="text-[1.0625rem] font-medium text-(--fg)">{t(lang, "about.savePage")}</p>
                    <p className="mt-0.5 text-[0.9375rem] text-(--fg2)">{t(lang, "about.savePageDesc")}</p>
                  </div>
                  <div>
                    <p className="text-[1.0625rem] font-medium text-(--fg)">{t(lang, "about.viewSavedPages")}</p>
                    <p className="mt-0.5 text-[0.9375rem] text-(--fg2)">{t(lang, "about.viewSavedPagesDesc")}</p>
                  </div>
                </div>
              </div>

              <div className="mb-6">
                <p className="mb-3 text-[0.75rem] font-semibold uppercase tracking-widest text-(--fg3)">{t(lang, "about.displaySection")}</p>
                <div className="flex flex-col gap-3">
                  <div>
                    <p className="text-[1.0625rem] font-medium text-(--fg)">{t(lang, "about.theme")}</p>
                    <p className="mt-0.5 text-[0.9375rem] text-(--fg2)">{t(lang, "about.themeDesc")}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className={panelCls(settingsSubView === "language")}>
            <div className="flex items-center justify-between border-b border-border px-5 pb-3 pt-2">
              <div className="flex items-center gap-2">
                ...
                <span className={`${scaleFont ? "text-[1.25rem]" : "text-[0.8125rem] tracking-[2px] uppercase"} font-semibold`}>{t(lang, "settings.language")}</span>
              </div>
              <Button size="icon-sm" variant="ghost" className="rounded-full bg-(--bg2)" onClick={onClose}>
                <X className="size-4" />
              </Button>
          ...
          <div className={panelCls(settingsSubView === "install")}>
            <div className="flex items-center justify-between border-b border-border px-5 pb-3 pt-2">
              <div className="flex items-center gap-2">
                ...
                <span className={`${scaleFont ? "text-[1.25rem]" : "text-[0.8125rem] tracking-[2px] uppercase"} font-semibold`}>{t(lang, "settings.installApp")}</span>
              </div>
              <Button size="icon-sm" variant="ghost" className="rounded-full bg-(--bg2)" onClick={onClose}>
                <X className="size-4" />
              </Button>
                ...
                {([1, 2, 3, 4] as const).map((step) => (
                  <div key={step}>
                    <div className="mb-3 flex items-start gap-3">
                      ...
                      <p className="text-[0.9375rem] leading-snug text-(--fg)">{t(lang, `installApp.step${step}`)}</p>
                    </div>
                    <Image
